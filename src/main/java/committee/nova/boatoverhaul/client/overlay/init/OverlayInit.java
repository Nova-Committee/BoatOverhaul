...
public class OverlayInit {
    ...
    public static void renderBoatOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        matrix.pushPose();
        startRender();
        renderGearScale(boat.getTargetGear(), graphics, overlay, gearXOffset, gearYOffset);
        renderGearCursor(boat.getGearState().getGear(), (int) (10F * boat.getGearAccumulation() / boat.getMaxGearAccumulation()), graphics, overlay, gearXOffset + 25, gearYOffset + 42);
        if (boat.getTargetRudder() != null && (boat.getTargetRudder() != Rudder.ZERO || boat.getTargetRudder() != boat.getRudderState().getRudder())) {
            renderRudderScale(boat.getTargetRudder() == Rudder.ZERO ? null : boat.getTargetRudder(), graphics, overlay, rudderXOffset, rudderYOffset);
            renderRudderCursor(boat.getRudderState().getRudder(), (int) (25F * boat.getRudderAccumulation() / boat.getMaxRudderAccumulation()), graphics, overlay, rudderXOffset + 9, rudderYOffset + 12);
        } else if (boat.isRudderWorking()) {
            renderRudderScale(boat.getRudderState().getRudder(), graphics, overlay, rudderXOffset, rudderYOffset);
            renderRudderCursor(boat.getRudderState().getRudder(), (int) (25F * boat.getRudderAccumulation() / boat.getMaxRudderAccumulation()), graphics, overlay, rudderXOffset + 9, rudderYOffset + 12);
        }
    ...
    private static void renderGearScale(Gear target, GuiGraphics graphics, ResourceLocation tex, int x, int y) {
        for (final IGearWidget g : GearWidgets.values()) {
            if (g.isForSelected() != (g.getRelativeGear() == target)) continue;
            graphics.blit(tex, x, y + g.getYOffset(), g.getStartX(), g.getStartY(), g.getWidth(), g.getHeight(), 256, 256);
        }
    }

    private static void renderRudderScale(Rudder target, GuiGraphics graphics, ResourceLocation tex, int x, int y) {
        for (final IRudderWidget r : RudderWidgets.values()) {
            if (r.isForSelected() != (r.getRelativeRudder() == target)) continue;
            graphics.blit(tex, x + r.getXOffset(), y, r.getStartX(), r.getStartY(), r.getWidth(), r.getHeight(), 256, 256);
        }
    }

    private static void renderGearCursor(Gear currentGear, int accumulationOffset, GuiGraphics graphics, ResourceLocation tex, int x, int y) {
        final IWidget g = CursorWidgets.GEAR_CURSOR;
        final int actualY = y - currentGear.getNumerator() * 10 / 4 - accumulationOffset / 4;
        graphics.blit(tex, x, actualY, g.getStartX(), g.getStartY(), g.getWidth(), g.getHeight(), 256, 256);
    }

    private static void renderRudderCursor(Rudder currentRudder, int accumulationOffset, GuiGraphics graphics, ResourceLocation tex, int x, int y) {
        final IWidget r = CursorWidgets.RUDDER_CURSOR;
        final int actualX = x + currentRudder.getNumerator() * 25 / 4 + accumulationOffset / 4;
        graphics.blit(tex, actualX, y, r.getStartX(), r.getStartY(), r.getWidth(), r.getHeight(), 256, 256);
    }
